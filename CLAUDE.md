...
- **Superseded storage model — migration 006 / ADR-001.** A custom field is no longer a row in `custom_field_defs` and a dynamic column in `contact_custom_values` (that table is retired). Definitions remain rows; `custom_field_values` holds one durable row per `(contact_id, field_def_id)` pair. Say "custom field," not "custom column."
- **Superseded dynamic-column TEXT rule — migration 006 / ADR-001.** The retired table no longer has one TEXT column per field. The normalized `custom_field_values.value` column is raw TEXT and is never rewritten merely to satisfy a type; never replace it with INTEGER, REAL, or BOOLEAN storage.
- **Field type lives in `custom_field_defs.type` and drives the UI widget only.** Storage type and field type are different concepts. Changing a field's type must not rewrite current custom-field values.
- **Route every sort or filter on a custom field through the single `sortExpr()` helper.** It is now a static expression over the normalized value column; raw TEXT still makes numeric ordering observable there. Never interpolate a custom identifier into an ORDER BY or comparison directly. `sortExpr()` currently has no runtime consumer, but this is still the required route for any future one.
- **Superseded dynamic-column index/UNIQUE ban — migration 006 / ADR-001.** The retired table's `DROP COLUMN` hazard no longer applies. `custom_field_values.uid UNIQUE` and `UNIQUE(contact_id, field_def_id)` are required integrity constraints; do not add arbitrary value semantics beyond those documented constraints.
...
- **Every destructive operation snapshots to `field_history` inside the same transaction.** Type conversions and permanent definition deletion both qualify. This is a bounded local audit trace, not backup or a recovery mechanism; there is no server and no backup.
- **Superseded dynamic-column deletion rule — migration 006 / ADR-001.** Deleting a definition removes dependent normalized value rows (by cascade/explicit deletion) in the same transaction; it does not perform `DROP COLUMN` DDL.
