...
**Depends on:** None
**Required by:** ADR-018 (Archive-Gated Contact Purge with Explicit Fan-Out); ADR-019 (Native Stack Contact Lifecycle Navigation); ADR-020 (Library-Only Photo Capture with Themed In-App Cropping and One-Time URL Download); ADR-022 (Tokenized Deterministic Initials Avatars); ADR-042 (Shared Status Palette for Dashboard and Widget Rings); ADR-047 (App-Level Assignable Sun and Themed Self Identity)
