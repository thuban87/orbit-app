...
**Required by:** ADR-063 (Versioned Lifecycle Backup and Dormant-Cadence Restore); ADR-066 (Deliberate Reviewed Import with Unbound Bulk Defaults); ADR-068 (User-Triggered, Source-Only Reconciliation with Durable Review); ADR-093 (Scoped Composable Dashboard Population and Filter Model); ADR-097 (Scoped Dashboard Search and Dedicated Unbound Retrieval); ADR-111 (Cadence-Guarded Profile Metrics and Composed Relationship Actions); ADR-119 (Reusable Count-Only History Aggregation and Canonical History Read); ADR-131 (Progressive Contact Creation and Complete-Record Editing); ADR-147 (Derived Digest Composition and Canonical Contacts Drill-Through).
