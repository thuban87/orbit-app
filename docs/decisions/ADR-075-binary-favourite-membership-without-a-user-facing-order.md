...
**Required by:** ADR-082 (Universal Capture FAB, Canonical Picker, and Truthful Quick Log); ADR-093 (Scoped Composable Dashboard Population and Filter Model); ADR-096 (Dashboard Header and Overflow Discovery Paths); ADR-098 (Scan-First, Accessible Dashboard List Rows); ADR-101 (Avatar-First Accessible Dashboard Card Renderer); ADR-102 (Frozen-Universe Dashboard Multi-Select); ADR-103 (Atomic Composed Dashboard Bulk Mutations)
