...
**Required by:** ADR-026 (Rogue Status for Unresponsive or Far-Overdue Contacts); ADR-032 (Flat Dashboard Discovery and In-Query Contact Search); ADR-034 (Birthday Banner and Re-query Dashboard Freshness); ADR-039 (Pre-Scheduled Inexact Decay Reminders); ADR-042 (Shared Status Palette for Dashboard and Widget Rings); ADR-043 (Static Globally Mirrored Favourites Widget); ADR-045 (Event-Driven Widget Refresh and Boot Recovery); ADR-046 (Query-Time Orrery Placement and Transactional Ring Ordering); ADR-093 (Scoped Composable Dashboard Population and Filter Model); ADR-104 (Durable Orrery Preferences and Live System Scope)
