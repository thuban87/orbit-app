...
Contact Import lets a user deliberately bring selected Android system contacts into Orbit without granting broad address-book access. It snapshots an Android-17 picker result into on-device SQLite, then drives reviewed single or incremental bulk import from that durable local state; it never treats the source provider as Orbit's authority.
...
|---|---|---|
| Native picker | `modules/orbit-contact-picker/android/src/main/java/expo/modules/orbitcontactpicker/OrbitContactPickerModule.kt` | Gates Android 17, launches the privacy-preserving picker, and copies selected fields from the temporary grant. |
...
1. `AddSpeedDialFab` or Settings calls the native availability probe; Android below API 37 keeps manual creation usable and does not create a session.
2. `OrbitContactPickerModule` launches the Android system picker without `READ_CONTACTS`, snapshots only allowed fields, and copies a selected photo from the temporary grant into app-owned cache.
3. `acceptPickedContacts()` moves accepted photos into flat private `import-staging/` paths, then commits the import session and every row together.
4. Cancelling before acceptance writes neither a session nor an Orbit contact.
...
2. The DAO rejects an invalid non-null birthday before opening its transaction; the screen keeps invalid raw input visible and disables Import.
...
2. `runImportBatch()` processes each safe row independently; ambiguous rows persist candidate evidence for later review and cannot block safe rows.
3. `ImportCompleteScreen` reads durable Imported, Already in Orbit, Need review, and Failed/skipped counts. Retry targets only genuinely failed rows; a completed batch can open Unbound contacts.
...
|---|---|---|---|
| Supported Android API | `37+` | `OrbitContactPickerModule.kt` | Enables the privacy-preserving system Contact Picker path. |
| Picker selection limit | `100` | `OrbitContactPickerModule.kt` | Requests the platform's supported multi-select limit without imposing an Orbit storage cap. |
...
2. **Do not add `READ_CONTACTS` or `QUERY_ALL_PACKAGES`.** The native module relies on the Android-17 picker and scoped manifest queries instead.
