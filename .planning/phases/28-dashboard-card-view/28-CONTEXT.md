...
- **D-13 (per-action eligibility — owner decision, 2026-09-06, via plan-review-convergence):** Every bulk action applies to **all selected contacts regardless of tracking state** — no per-action eligibility filtering for Unbound (`tracking_enabled = 0`) or never-contacted contacts. Applying bulk Set Frequency / Snooze to such contacts is schema-legal (migration-011 `CHECK (tracking_enabled = 0 OR interval_days IS NOT NULL)` is satisfied by any positive value) and is the intended, least-surprising behavior, matching the single-contact actions. Per-action greyout/gating is explicitly **out of scope** for this phase (additive follow-up only). Realized by 28-07 (the apply-to-all bulk composers; no eligibility filter).
