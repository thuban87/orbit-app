...
- **D-13:** OpenRouter OAuth anti-CSRF `state` is **REQUIRED, not conditional** (owner decision, cycle-2 convergence). The callback validator MUST reject a redirect whose `state` is missing OR mismatched **before** code exchange — Orbit always sends `state`, so a callback without a matching echoed `state` is rejected (not accepted-with-PKCE-fallback). If the Pixel device-UAT spike finds OpenRouter does not echo `state` back, the OpenRouter connect fails and the posture is revisited as an explicit new decision — loosening to PKCE-primary is NOT a silent implementation choice. This strengthens (never weakens) the D-05 egress/OAuth security posture; reviewer-flagged control downgrades at this boundary are owner escalations.
- **D-14:** **Profile presentation IS included in the v5 backup — the coordinated backup-format decision that `docs/systems/profile.md:82` and `backup-restore.md:123` explicitly deferred to Phase 36 (owner decision, cycle-3 convergence).** 36-08 must serialize AND restore the per-contact/per-category Profile presentation entities — `profile_contact_presentation` and `profile_category_presentation` (template + background assignments, `freeform_layout_json`, `collapse_json`) — **and** the background image bytes (staged like contact photos' `photoBase64`, not just `image_path`). Restore uses parent-before-child UID remap (assignments reference contacts/categories/templates by portable UID, never raw rowid) with a different-rowid destination roundtrip test, so a restored install loses NO profile presentation state (AICFG-16/17 no-silent-data-loss bar). This RESOLVES the deferral (does not reverse it); when the phase executes, update `docs/systems/profile.md` and `docs/systems/backup-restore.md` to record that presentation + background bytes are now v5 entities (retire the "device-local pending" boundary note).
