...
- **D-01 [informational]:** Process directive to planning agents, not a plan-tracked feature (honored by the planner/checker: dossier read in full, trip-wires respected). Read the phase dossier (canonical_refs) IN FULL before planning. Where present, its dated "Amendment — audit resolutions 2026-09-01" section overrides older text. [DECIDED] and [REJECTED] items are settled: reopening one, or reversing any Accepted ADR or HANDOFF.md entry, is an owner decision — stop and ask, never "fix" it.
- **D-02 [informational]:** Process directive to planning agents, not a plan-tracked feature (honored: planning-notes + phase-08-planning-notes read; E-02/E-03 constraints carried into the resolver base scope). Read the phase planning-notes file (canonical_refs) as a binding appendix: every REPLAN finding must be reflected in the plan, and every trip-wire is a stop-and-ask. This dossier carries **no** dated 2026-09-01 amendment block and no auto-fixes; all body decisions stand as originally written, and no escalation routed here — read `phase-08-planning-notes.md` first for the E-02/E-03 constraints this phase inherits.
...
- **D-11 [owner clarification, 2026-09-08]:** ORRS-13 remains incomplete. The polished normal-motion switch is an observable staged choreography: the current System gains rotational impulse and visibly accelerates; leaving contacts peel/spin outward and shed away; retained contacts preserve continuity; entering contacts stream/spin inward and are captured into destination orbits; then the destination decelerates and settles into canonical geometry. Phase 29's 260 ms fade/shrink/grow-at-final-position continuity plus a small reversible rotation does **not** satisfy this decision. Membership delta scales the choreography's drama. Reduced Motion alone replaces it with the simple crossfade/reposition path. Exact constants remain implementation/device-tuning, but Phase 30 cannot be marked complete until the owner approves the complete normal-motion choreography on the physical Pixel.
