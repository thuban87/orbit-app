...
- **D-12 (search-result ordering — owner decision, cycle-1 convergence escalation 2026-09-05):** List View search results are ordered **relevance-first, per recorded requirement DASHQ-09** — corpus matches ranked by term coverage → match score, with the resolved Dashboard sort as **tie-breaker only** (exactly what the shipped `searchDashboard()` engine in `src/logic/dashboard-search-match.ts` produces). Plan 06's original `composeDashboardSearch()` design — re-emitting matched rows in Dashboard (eligible) order and using relevance only for membership — **inverted DASHQ-09 and is rejected**; the composition MUST preserve `searchDashboard()`'s relevance ordering rather than re-sorting into Dashboard order. The planner must also define how **fuel-only matches** (which carry `match === null` and no relevance score) merge into the relevance-ordered corpus results — a defined, tested merge rule, not incidental ordering. DASHQ-09 stands unchanged; this decision enforces it, it does not reverse it.
