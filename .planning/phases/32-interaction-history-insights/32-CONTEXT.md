...
- **D-12 [enforcement — added 2026-09-11, plan-review convergence cycle 1]:** Enforces D-07; makes no new decision. Migration 025 (Phase 32) adds ONLY `duration`, `allow_ai`, `history_lens`, `history_cycle_count` and the Tone/channel value remap. It **must NOT** add `group_event_id` or any group-event schema — that column lands in Phase 33's migration (026+) as one strictly ordered sequence (D-07; roadmap Schema note). Consequently **all group-linked behavior in Phase 32 (Plans 03/07: parent-excluded counts, restrained Group Event context, `GroupScopePrompt`, Edit/View Group Event routing) is an inert seam**: the group-link discriminator always resolves false in Phase 32 because no `group_event_id` column exists yet, so group branches are structurally present but never exercised (dossier lines 66/168 scope Phase 32 to the seam; Phase 33 owns group persistence). Any Phase-32 test fixture that requires a constructible Group-Event parent+child pair (e.g. Plan 03 Task 1's "group-parent-plus-child" fixture) must be reframed to assert the seam's inertness (canonical child interactions only; no parent row/table), **not** satisfied by introducing group schema early. **Trip-wire:** making the fixture "constructible" by adding `group_event_id` to migration 025 REVERSES D-07 — stop and ask the owner, never do it silently.
