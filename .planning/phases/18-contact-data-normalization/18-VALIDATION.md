...
| 18-03-02 | 18-03 | 2 | CDN-02, CDN-03 | T-18-09 | Dashboard/header counts, shared status fragments, digest, favourites, and Orrery exclude Unbound while search remains retrieval-oriented and sun policy falls back through one predicate. | SQL/logic read unit | `npm test -- src/db/dashboard-read.test.ts src/db/orrery-read.test.ts src/db/contact-status-read.test.ts src/db/digest-read.test.ts src/db/status.test.ts src/db/queries.test.ts src/db/sun-picker-read.test.ts src/logic/sun-occupant-logic.test.ts` | Existing files extended | ⬜ pending |
...
| 18-07-03 | 18-07 | 4 | CDN-04 | T-18-23 | Explicit tombstones alone remove normalized children; failed graph validation is atomic. | Restore/schema integration | `npm test -- src/backup/backup-schema.test.ts src/backup/restore-apply.test.ts src/backup/reconciliation.test.ts` | Existing files extended | ⬜ pending |
...
- [ ] 18-03/18-05/18-09 Waves 2-4 extend lifecycle population reads, shared status/queries fragments, sun policy/candidates, settings, Home, notification, widget, and quick-action guard owner tests; 18-11 Wave 3 separately extends the nullable-cadence impact and explicit-AI context owners. 18-05-02 specifically extends `src/db/dashboard-read.test.ts` for default Bound-only and persisted opt-in list/count parity.
- [ ] 18-07/18-10 Waves 3-4 extend backup, reconciliation, and purge tests and create the narrowed share/export plus Backup screen tests before changing their contracts.
