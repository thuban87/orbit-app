...
- ✅ **Settings "Weekly digest" toggle (DGST-01 toggle half)** — present as the 4th notification type (after Decay reminders + Birthday alerts, before "Show names on lock screen"), **ON by default**, gentle helper "A Sunday-morning look back at your week — who you reached, and who's slipping quietly.", token-styled. Flips true→false→true correctly (each flip fires `reconcileDigestSchedule` per H1 — schedule effect not inspectable on a non-run-as-able release APK).
