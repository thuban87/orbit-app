...
- **D-11:** Two owner reconciliation blockers **RESOLVED by owner 2026-09-12** (were: do not fabricate; now settled — use these verbatim, do not re-flag):
  - **Log naming (D-13-184):** the canonical form/CTA name is **"Log Interaction"** (matches dossier §AN normalization direction and the provisional UI-SPEC). "Log Contact" is legacy — do not use.
  - **Default/general built-in Memory type display name (D-13-060 / D-RM-056 / D-13-190):** final display name for the `general` type is **"Memory"**, and the `custom` type is renamed **"Custom"** to avoid a duplicate label (verified against `src/db/memory-registry.ts`: `general` currently shows `PROVISIONAL_DEFAULT_MEMORY_TYPE_NAME = "General"`; `custom` currently shows `PROVISIONAL_MEMORY_LABEL = "Memory"`). Concretely: `MEMORY_TYPE_REGISTRY.general.displayName` → "Memory" and `MEMORY_TYPE_REGISTRY.custom.displayName` → "Custom" (the planner decides whether to retire/repoint the two provisional constants or set the values directly). Rapid capture still requests the type by registry key (`DEFAULT_MEMORY_TYPE_KEY = "general"`), never by hardcoded display name or type ID.
