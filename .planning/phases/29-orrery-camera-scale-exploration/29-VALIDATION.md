...
| Automatic-backup contention and cancellation | ORRC-07, ORRC-15 | Native delay/interaction observation | Tap a contact during normal automatic backup, then repeat with dismissal/Recenter, System switch and background while validation waits; verify no late action. Record actual overlap, fixture scale and evidence; unobserved overlap stays pending. |
| Scene snapshot versus app write | ORRC-01, ORRC-04 | Native responsiveness under shared mutex | Observe scene refresh concurrent with Quick Log/write; record completion and visible wait without changing FIFO/coherence or inventing a busy surface. Separate wall-clock wait from frame timing. |
...
## Cycle 2 validation additions — 2026-09-07

Task IDs, waves, threat IDs and automated commands are unchanged. 29-04-01 now has its explicit acceptance block matching parity/full-history/chunk-count/read-only/snapshot/render criteria, plus queued-generation cancellation. 29-08-01 adds deterministic deferred-mutex cancellation and live-action controls. 29-12-01 uses real buildExportManifest with a deferred photo reader and an instrumented scene read to exercise export→scene/action and scene→recency-write ordering without sleep-based timing assertions. Existing owning test commands run these added cases during execution.

29-05-01 source-tree inspection verifies a consecutive body-Group batch; native occlusion is still a separate obligation. 29-12-02 verifies creation-owned source citations and records ADR-027's precise partial-supersession KB handoff in the living doc and SUMMARY, retaining all derivation/raw-score/intensity policies. Native wait time and long-history/photo-library contention remain unmeasured; D-10 assigns measured optimization to Phase40. The added observations do not impose a new timeout, priority, busy UI or approval gate. Fresh independent checking/review is still required; no implementation/native result is claimed.
