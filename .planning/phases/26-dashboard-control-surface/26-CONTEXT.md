...
- **D-12 (search read seam — owner decision 2026-09-05, resolves RESEARCH O-1):** Dashboard search in the new Phase 25 query world is served by a **new, additive population-aware search read authored in Phase 26** that composes population + filters + a search term and **preserves the recorded A3 search-scope semantics** (`dashboard-read.ts:38` — a term relaxes the never-contacted/snooze exclusions to archived-only; the no-term list keeps the full exclusion). It **must not** add a `term` to, or otherwise fork, Phase 25's `listDashboardPopulation` (no layer-1 fork — D-11). The legacy `listDashboard` search read is retired in this phase once the new read is wired; **no dual-read** is left in place. Search-scope semantics are a recorded owner decision — do not silently narrow or widen them. Unbound retrieval (D-08/ADR-062) is a separate path and still gets its own child-route search.
