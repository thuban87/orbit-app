...
- **D-03:** This phase ships SQLite schema. Never assume a migration number — verify head+1 against `src/db/migrations/` and `TARGET_VERSION` in `src/db/database.ts` on disk at plan time (numbers drift every schema phase). Live `BACKUP_FORMAT_VERSION` is already 4, so v4 is spent: Phase 31 does not bump or widen wire-format emission. It adds only its durable `app_settings` preference keys to the portable-settings declaration/snapshot allowlist, never AsyncStorage. Phase 36 owns the coordinated closing v5 — or an owner-decided extend-v4-in-place — for all new template/assignment/image entities and referenced files.
...
- **D-12:** Ordinary Off Limits items remain rows in `fuel`, whose live schema/read/write/backup paths have no `allow_ai` field. Phase 31 renders them with the settled caution semantics and **omits the sparkle** because the combined state is unrepresentable. Preserve an adapter/model seam that can accept a future explicit permission only after an owning phase adds durable storage; never infer permission from visibility, kind, source, or location. Phase 31 must not add an Off Limits `allow_ai` migration or widen AI egress. (Owner ruling, 2026-09-09.)
