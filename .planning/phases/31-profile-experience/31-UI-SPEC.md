...
Off Limits uses the avoid/caution treatment and the helper `Avoid bringing these up`. It is not collapsed merely because content is sensitive. Ordinary Off Limits has no durable permission field, so Phase 31 omits its sparkle while preserving a renderer seam for a future explicit permission; the visual never infers permission from hidden, kind, source, or location state.
...
|--------|-------------------------|
| `31-CONTEXT.md` | All D-01 through D-12, including AI-entry removal, nullable-cadence guard, durability, reset boundary, Profile background permission, replaceable History seam, and Off Limits no-sparkle/no-inference ruling |
