...
| THEME-11 | Every accent/text/status/danger+onDanger passes AA in all 4 combos (split gate: hard-fail new palettes incl. the named `danger`/`onDanger` pair, flag-for-owner legacy galaxy-dark incl. its onDanger/danger pair); accent-id set === `ACCENT_IDS` (no drift) | unit | `npx vitest run src/theme/contrast.test.ts src/theme/accents.test.ts` | ❌ W0 (new `contrast.ts` + test; `accents.test.ts` in 23-03) |
...
| THEME-09 | registry maps every semantic name (incl. all four TabParamList route keys DashboardTab/OrreryTab/BackupTab/SettingsTab via the explicit mapping, six status names, warning); screens use names only; `Icon.tone` typed to string-valued `IconTone` (array-valued key rejected via `@ts-expect-error`) | unit | `npx vitest run src/components/icons/icon-registry.test.ts` | ❌ W0 |
| THEME-04 | `resolveBackground`/`resolveRenderableBackground` (NULL→default, 'none'→solid, render-fail→None/Solid); slot-id set === `BACKGROUND_SLOT_IDS` (no drift) | unit | `npx vitest run src/theme/backgrounds.test.ts` | ❌ new (23-06) |
...
| Per-asset brightest-region text-over-glass legibility (each Galaxy background) | THEME-11 / THEME-05 | The composited unit test validates only the DECLARED worst-case pixel in `backgrounds.ts`; nothing decodes the shipped `.webp`, so on-device against the decoded image is the only check of the actual asset bytes (REVIEWS 23-06 cycle-4 MEDIUM) | On the Pixel, for EACH Galaxy background asset, place body + caption text over glass on the asset's visibly brightest region and confirm it stays legible; a failure means the shipped asset exceeds its declared worst-case pixel — re-master/darken the asset or retune its declared pixel + tint opacity, never weaken AA |
