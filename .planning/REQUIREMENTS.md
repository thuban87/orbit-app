...
- [ ] **AICFG-08**: AI-permitted Contact Knowledge is included automatically per contact; Message Focus adds emphasis without changing permission; Off Limits is never transmitted to AI in any form (ADR-107/D-14 reverses ADR-078's off-limits egress; the Research "Avoid" display remains); the three most recent Interactions may be included with a note only where that interaction's Allow AI toggle is ON; Group Notes are never transmitted
