...
- **ADR-075–080 record the audit-ratified reversals** produced by the 2026-09-01 cross-dossier audit
  ...
  Orrery view, Off Limits never sent to AI (ADR-107 reverses ADR-078's off-limits egress), retired exact-prompt first-send
...
- [x] 29-05-PLAN.md — Shared projected frame, billboard depth and semantic labels

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 29-06-PLAN.md — Measured shell obstacles and reachable controls

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 29-07-PLAN.md — Camera gestures, Polaris and bounded Recenter

**Wave 8** *(blocked on Wave 7 completion)*

- [x] 29-08-PLAN.md — Single/group focus and accessible companion

**Wave 9** *(blocked on Wave 8 completion)*

- [x] 29-09-PLAN.md — Deliberate contacted-only guarded reorder

**Wave 10** *(blocked on Wave 9 completion)*

- [x] 29-10-PLAN.md — Subordinate relationship satellites

**Wave 11** *(blocked on Wave 10 completion)*

- [x] 29-11-PLAN.md — Session return, Reduced Motion and race-safe recovery

**Wave 12** *(blocked on Wave 11 completion)*

- [x] 29-12-PLAN.md — Integrated verification and native evidence handoff
...
**Plans**: 12/12 plans executed (8 waves); UAT 5/5, verification 5/5, security gate clear, and owner approval recorded 2026-09-09 — Plans 30-11/12 corrected ORRS-13 by restoring the owner-approved staged spin/shedding/capture choreography after Plan 30-10 delivered only the Phase-29 continuity effect plus a small reversible spin
**Wave 1**

- [x] 30-01-PLAN.md — TRACER: migration 022 (Systems table set, one-way-door checkpoint) + thin manual-only custom System end-to-end (union/grammar/resolver/read/switcher) + custom-routing-before-buildWhere guard + missing-custom read result + cross-catalog name uniqueness [ORRS-01/02/12]

**Wave 2** *(blocked on Wave 1)*

- [x] 30-02-PLAN.md — Custom-System membership resolver: rules (OR/AND) + Gravity TS pass + inc/exc pruning + broken-rule detection (ruleUid) + scope:population base-mode + draft-vs-persisted resolvers + stable ordering [ORRS-01/02/04]
- [x] 30-03-PLAN.md — systems-dao full CRUD: rename/delete+fallback/duplicate(All Contacts→scope:population)/reorder/hide-show(catalog-validated)/overrides(setSystemOverride)/reset + atomic saveSystemDefinition + uid-preserving Undo + prune + cross-catalog CI-unique names + immutability [ORRS-02/03/09/10]
- [x] 30-04-PLAN.md — Backup declare-only: restore-accept custom:<uid> (pinned vs types.ts) + Systems entity contract for Phase 36 (no wire emission, no format bump) [ORRS-14]

**Wave 3** *(blocked on Wave 2)*

- [x] 30-07-PLAN.md — Manage Members: searchable virtualized multi-select grid (preselect/exclude-in-place/add/counts) on the picker LOGIC, local photo path, setSystemOverride intent [ORRS-02/06]
- [x] 30-10-PLAN.md — Switch animation + camera: Home framing + shared-focus preservation (replaces the unconditional focus-clear) + stash/diff membership delta + delta-adaptive spin/shed/capture in OrreryWorld + reduced-motion crossfade + startup missing-custom fallback [ORRS-12/13]

**Wave 4** *(blocked on Wave 3)*

- [x] 30-08-PLAN.md — Builder HUD wizard: Definition accordions (8 axes) + draft live count (resolveMembershipFromDefinition) + embed Manage Members + atomic save-switch semantics + SystemBuilder route in BOTH stacks + whole-canvas inert + discard guard [ORRS-01/02/05/08]

**Wave 5** *(blocked on Wave 4)*

- [x] 30-06-PLAN.md — Systems Management screen + routes (Orrery + Settings) + Create/Edit → SystemBuilder (both stacks) + contact-safe delete+Undo + active-delete persists All-Contacts fallback [ORRS-03/04/09/10]
- [x] 30-09-PLAN.md — Full-canvas Preview: simplified render over the shared Phase 29 scene derivation + focus/AppState lifecycle + Preview/Edit bar + Save-from-Preview [ORRS-07]

**Wave 6** *(blocked on Wave 5)*

- [x] 30-05-PLAN.md — Switcher extension: management order/visibility/live counts (bounded)/empty-vs-broken severity + OrreryScreen catalog data-flow owner + Manage Systems entry + 3 icon keys [ORRS-11]

**Wave 7** *(blocked on Wave 6)*

- [x] 30-11-PLAN.md — Corrective choreography engine: explicit accelerate/shed/capture/settle phases, retained/leaving/entering trajectories, delta-scaled drama, interruption/pause semantics, and deterministic motion-property tests [ORRS-13]

**Wave 8** *(blocked on Wave 7)*

- [x] 30-12-PLAN.md — Live Skia integration with lifecycle-surviving state and continuous Home framing; physical-Pixel tuning/evidence, debug install via temporary 8082 reverse, release APK on droid, and blocking owner approval [ORRS-13]
...
- [x] 31-14-PLAN.md — Direct layout drag plus shared template discovery and arbitrary contact assignment
...
- [x] 31.1-01-PLAN.md — TRACER: per-package background setter + single shell BackgroundHost mount + transparent nav theme + focused-route density + Home cooperates + Orrery excluded (device-proven)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 31.1-02-PLAN.md — Inline grouped Background picker in Settings → Appearance (all 8 + None/Solid, per-package write) + Settings root cooperates
- [x] 31.1-03-PLAN.md — Cooperating screen roots: flip 30 ordinary screen roots transparent (Orrery + Profile excluded), grouped by density tier

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 31.1-04-PLAN.md — Full regression + D-19 B physical-Pixel device-UAT matrix + Profile precedence/regression + standalone release-APK native gate
...
- [x] 32-08-PLAN.md — Profile History section integration (replace timeline) + empty-date logging route + empty state (wave 4)
...
- [x] 33-02-PLAN.md — ContactPicker multi-select + Group Event route types (W1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 33-03-PLAN.md — Live inheritance logic + shared/date edit fan-outs + per-field override (W2)
- [x] 33-04-PLAN.md — Group Events read layer (browse, search, detail, participant resolution) (W2)
...
- [x] 33-06-PLAN.md — Group Log create form + Edit Group Event form + participant override editor (W4)

**Wave 5** *(blocked on Wave 4 completion)*

- [x] 33-07-PLAN.md — Group Event Detail + browse UI + history seam activation (scope prompt, convert, group block) (W5)
...
**Plans**: 8/8 plans executed across 3 waves
**Wave 1**
...
- [x] 34-08-PLAN.md — Edit Contact IA: top-level accordion sections composing every editor + reveal-and-focus + dirty-state (CAPT-04/14)
...
**Success Criteria** (what must be TRUE):
  ...
  4. User can open Things to Remember Research as a sibling full-screen side showing a compact read-only projection of only populated conversation-relevant knowledge (including Key People) with no add/edit actions, mark up to three AI-authorized items Add to AI as session-only Message Focus shown compactly on the Compose side (never granting permission), and see Off Limits in a distinct Avoid presentation that can never be Message Focus and is NEVER sent to AI in any form — not as positive context nor as a negative avoidance constraint (D-14/ADR-107 reverses ADR-078's off-limits egress) (COMP-08/10/11)
...
**Plans**: 6/9 plans executed (waves 1–5)
...
- [x] 35-06-PLAN.md — Research side (read-only projection) + session-only Message Focus (≤3, no permission)
...
**Success Criteria** (what must be TRUE):
  ...
  4. What is transmitted is exactly what the permission model authorizes — AI-permitted knowledge included automatically per contact, Message Focus adding emphasis without changing permission, Off Limits never transmitted to AI in any form (ADR-107 reverses ADR-078's off-limits egress; the Research "Avoid" display remains), the three most recent Interactions included only where that interaction's Allow AI is ON, and Group Notes never — governed by a central permission manager (new-item type defaults OFF and new-items-only, searchable review, contact drill-in, bulk disable, bulk enable only behind explicit impact confirmation, Group Notes never surfaced), inspectable and previewable in Settings without ever showing credentials, and disclosed once on first successful setup with the exact-prompt first-send acknowledgement retired (AICFG-08/10/11/12)
...
**Plans**: 11/11 plans executed (7 waves) — 36-10 closes the verifier-found AICFG-08 Message Focus payload gap; 36-11 closes the OpenRouter callback gap; automated verification and all seven device/visual checks are complete
Plans:
...
- [x] 36-02-PLAN.md — OpenRouter lane: OAuth PKCE browser-auth (+ anti-CSRF state/strict callback) + model catalog + live pricing (AICFG-04)
- [x] 36-03-PLAN.md — Prompt assembly: render carry-only egress (shared memories + gated notes, NO off-limits/ADR-107, no artificial ceiling) + Adjust + retire legacy AI-fuel confirm (AICFG-08/09/15/17)
- [x] 36-04-PLAN.md — Central AI permission manager + new-item defaults wired to the creation writers (AICFG-10)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 36-05-PLAN.md — Three-lane connection & model UI + Needs Attention/repair (AICFG-02/03/05/15)
- [x] 36-06-PLAN.md — Writing Style + Personalization Context (paste/import) + token/context/cost estimate vs real model capacity, explicit overflow (AICFG-06/07)

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 36-07-PLAN.md — Transparency (first-use disclosure + Preview + Compose review) + failure categories + sanitized diagnostics (AICFG-11/12/13/14)
...
- [x] 36-10-PLAN.md — Revalidate Message Focus permissions at generation time and add stronger emphasis to the exact immutable provider payload (AICFG-08)
...
- [x] 38-01-PLAN.md — Five-tab shell + semantic routing (tracer) [wave 1]
- [x] 38-02-PLAN.md — Your Week data layer + migration 030 + period preference [wave 1, has reversibility checkpoint]
- [x] 38-03-PLAN.md — Up Next + Horizon composition logic (up-next-read + digest-composition) [wave 1]

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 38-04-PLAN.md — Digest notification deep-link + global FAB audit + Contacts header cleanup [wave 2]
- [x] 38-05-PLAN.md — Your Week presentation (section + heatmap + inline day detail) [wave 2]

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 38-06-PLAN.md — Digest surface assembly (DigestScreen rewrite + Up Next + Horizon) [wave 3]
...
- [x] 38-08-PLAN.md — Events participant-detail → Profile repair + DEV-only Never Contacted/notification probes + focused Pixel UAT closure [wave 5, gap closure]
...
**Goal**: A bounded, post-Phase-38 polish pass that establishes durable presentation contracts across Orrery, Contact Profile, Interaction History, and Contacts before Phase 39 Onboarding is designed around those surfaces. Not a redesign and not open-ended cleanup — it fixes named presentation regressions and interaction friction, makes existing Profile knowledge genuinely editable using the current data model, and closes with a bounded audit for the same defect classes. Adds no new relationship-domain concepts and no schema/backup-format change.
**Depends on**: Phase 38 (cleans up the surfaces Onboarding will teach against)
**Requirements**: TBD — deferred to planning (see dossier §R planning-time verification checklist, 15 items)
**Success Criteria**: Defined at planning time; dossier §N UAT matrix executed on physical Android hardware
...
> **Ready to discuss — dossier authored 2026-09-19 (owner + codex), grounded against the post-Phase-38 repo 2026-09-19; premises confirmed. Run gsd-discuss-phase next, then plan against the dossier as the authoritative contract. Two items flagged during grounding to resolve in discuss: (1) the translucent Orrery-overlay treatment must be a NEW named token treatment in `src/theme/tokens/surface.ts`, AA-proven per `surface.test.ts`, not a `GlassSurface` fork or a change to ordinary content-card opacity — as-is it brushes ADR-087 "no parallel component families" / ADR-115's rejected "always-translucent cards"; (2) a manual compact/wide width toggle ALREADY exists in `ProfileLayoutEditor.tsx:149-164` for `orbit-status` + `last-interaction`, so dossier §I "do not add manual width controls" must reconcile keep-vs-remove and define how orphan-stretch interacts with an explicit compact choice.**
