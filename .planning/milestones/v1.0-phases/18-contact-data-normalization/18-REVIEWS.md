...
| ID | Severity | Finding | Incorporated in this replan |
|---|---|---|---|
| C3-H1 | HIGH | Orrery's `rewriteRingSeq` count/update guards omit the Bound predicate after `listOrbitingContacts` becomes Bound-only, so one contacted Unbound contact makes every reorder throw. | Incorporated: 18-12 Task 2 owns `ring-seq-dao.ts`/test and requires identical `tracking_enabled = 1` COUNT and scoped-UPDATE guards plus a contacted-Unbound reorder regression; 18-08 Task 1 expands the ledger to orbiting/favourite predicate consumers. |
| C3-H2 | HIGH | Migration 009 rebuilds `app_settings` and relationship children without a proof that all existing columns, defaults, indexes, UNIQUE constraints, and data survive. | Incorporated: 18-01 Task 3 seeds distinctive values for every pre-v9 app_settings column, asserts full row equality, and compares `PRAGMA table_info` plus sqlite_master table/index SQL for app_settings and each rebuilt child. It evaluates `legacy_alter_table` only as an optional smaller-blast-radius route; completeness proof remains mandatory. |
| C3-M1 | MEDIUM | The typed `MergeableEntityType`/`TombstoneEntityType` refactor can silently omit non-mergeable `field_history` from purge and Replace-all deletion. | Incorporated: 18-07 Task 2 requires typed maps plus an explicit exhaustively typed non-mergeable residual owned-table list containing `field_history`, preserves FK-safe order, and proves Replace-all removes seeded history. |
| C3-M2 | MEDIUM | Dashboard first-run detection has no Unbound count, so an all-Unbound user sees “Add your first contact” above an Unbound footer. | Incorporated: 18-05 Task 1 owns `dashboard-empty-logic.ts`/test and Home threading; it adds `unbound` to DashboardEmptyInput, requires all five populations to be zero for first-run, and tests zero/non-zero Unbound cases. |
| C3-M3 | MEDIUM | Included Unbound Never Contacted rows can render a stale favourite star and active `ContactCard` chrome; `DashboardRow.trackingEnabled` is omitted by the unchecked list cast. | Incorporated: 18-03 Task 2 makes `listNeverContacted` project `trackingEnabled` and NULL favourite rank for Unbound; 18-05 Task 2 owns `NeverContactedScreen.tsx`/test neutral treatment with no favourite or status ring. |
| C3-M4 | MEDIUM | Backup parsing admits illegal lifecycle/cadence combinations, so restore fails mid-transaction with a raw SQLite CHECK/trigger error instead of `BackupSchemaError`. | Incorporated: 18-07 Task 1 validates intervalDays/trackingEnabled/Bound cadence and removes retired v1 scalar keys; Task 3 adds negative parse and invalid-merge cases proving BackupSchemaError before apply. |
| C3-M5 | MEDIUM | Streamlined Export lacks a UI-spec copy/state contract, a distinct artifact identity, and device UAT despite being a new PII egress surface. | Incorporated: 18-UI-SPEC defines copy/state and non-backup identity; 18-10 Task 2 requires/test `orbit-streamlined-export-*.txt`, `text/plain`, and `Share Streamlined Export`; 18-08 Task 2 inspects it in device UAT. |
| C3-M6 | MEDIUM | DAO promotion cannot protect direct restore writes from two surviving primary methods of a type. | Incorporated: 18-01 Task 3 creates/tests the partial primary-method unique index; 18-07 Tasks 1/3 reject duplicate surviving primaries per `(contactUid, type)` before restore with backup negative coverage. |
