...
function photoFs(overrides: Partial<ImportedPhotoFs> = {}): ImportedPhotoFs {
  return {
    ...
    contactPhotoRelPath: async (contactId) => `avatars/contact-${contactId}.jpg`,
...
describe("source consolidation", () => {
  it("clusters selected rows by a shared canonical method, never name alone", async () => {
    const session = await createSession([
      { externalContactId: "one", sourcePayload: payload("Taylor", [{ type: "phone", value: "(312) 555-0100" }]) },
      { externalContactId: "two", sourcePayload: payload("Taylor", [{ type: "phone", value: "+1 312 555 0100" }]) },
      { externalContactId: "three", sourcePayload: payload("Taylor", []) },
    ]);

    expect(detectSourceClusters(session.rows, { phoneRegion: "US" })).toEqual({
      clusters: [expect.arrayContaining([expect.objectContaining({ externalContactId: "one" }), expect.objectContaining({ externalContactId: "two" })])],
  ...
  it("creates one Unbound contact, resolves every row atomically, and preserves birthday plus source identity", async () => {
    ...
    expect(await exec.getFirstAsync<{ count: number }>("SELECT COUNT(*) AS count FROM external_contact_links WHERE contact_id = ?", [result.contactId])).toEqual({ count: 2 });
    expect(await listSessionRows(exec, session.sessionId)).toEqual(expect.arrayContaining([
      expect.objectContaining({ rowStatus: "imported", contactId: result.contactId }),
      expect.objectContaining({ rowStatus: "imported", contactId: result.contactId }),
    ]));
    await expect(findActiveExternalLink(exec, "android", "one")).resolves.toBe(result.contactId);
    await expect(findActiveExternalLink(exec, "android", "two")).resolves.toBe(result.contactId);
  });
  ...
  it("rolls back contact and row resolutions for an in-transaction failure, while photo failure is post-commit only", async () => {
    ...
    const photoFailure = await createSession([
      ...
      { externalContactId: "photo-two", sourcePayload: payload("Taylor"), photoRelPath: "import-staging/taylor.jpg" },
    ]);
    const result = await combineCluster(exec, photoFs({ resizeToMaster: async () => { throw new Error("bad image"); } }), { rows: photoFailure.rows, batchCategoryId: null, phoneRegion: "US", now: NOW });
    expect(result).toEqual({ contactId: expect.any(Number), combined: true });
    expect(await listSessionRows(exec, photoFailure.sessionId)).toEqual(expect.arrayContaining([
  ...
  it("refuses an all-nameless cluster and leaves its rows pending", async () => {
    ...
    await expect(combineCluster(exec, photoFs(), { rows: session.rows, batchCategoryId: null, phoneRegion: "US", now: NOW })).resolves.toEqual({ combined: false, reason: "name-required" });
