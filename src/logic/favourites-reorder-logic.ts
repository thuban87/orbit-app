/**
 * Pure drag→order computation for Manage-favourites (DASH-06, Plan 08 UI half).
 *
 * Extracted as a `*-logic.ts` pure module (no RN/native imports, node-tested) —
 * the same split as `edit-contact-logic` / `create-contact-logic`. This owns the
 * array math of a drag move; the transactional persistence is
 * `rewriteFavouriteRanks` (favourites-dao). The drag-end handler computes the new
 * ordered id list here, then hands it to the DAO to write ranks 0..n-1.
 *
 ...
 * stray gesture index can't crash the reorder screen); a `from === to` move after
 ...
 *
 * @param orderedIds the current favourites order (id per slot)
