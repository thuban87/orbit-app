...
import { clampCameraPose, HOME_CAMERA, projectWorldPoint } from "./orrery-camera-logic";
import { inertiaTarget, northTarget, planRecovery, recoveryCurve, sampleRecovery, polarisProjection } from "./orrery-recovery-logic";
...
describe("bounded camera recovery", () => {
  it("zero/small/large recovery is finite, distance-adaptive, continuous and exact", () => {
    ...
    const large = planRecovery({ ...home, x: 500, y: -500, zoom: 4, tilt: 1, yaw: 3 }, home, extent, false);
    ...
    for (const plan of [zero, small, large]) {
      ...
      for (let i = 0; i <= 1000; i++) {
        ...
        previous = progress;
        expect(Object.values(sampleRecovery(plan, t)).every(Number.isFinite)).toBe(true);
      }
    }
  });
  it("north follows the shortest wrap and preserves every other axis", () => {
    const start = { ...home, x: 30, y: 20, zoom: 2, tilt: 0.5, yaw: 2 * Math.PI - 0.2 };
  ...
  it("Reduced Motion is short/direct and disables pan/yaw continuation; no zoom/tilt coast", () => {
    ...
    expect(inertiaTarget(home, "pan", 99999, -99999, extent, { width: 400, height: 600 }, true)).toEqual(home);
    for (const kind of ["pan", "yaw"] as const) {
      const next = inertiaTarget(home, kind, 99999, -99999, extent, { width: 400, height: 600 }, false);
      expect(Math.abs(next.x)).toBeLessThanOrEqual(18);
      expect(Math.abs(next.y)).toBeLessThanOrEqual(18);
      expect(Math.min(next.yaw ?? 0, 2 * Math.PI - (next.yaw ?? 0))).toBeLessThanOrEqual(0.08);
  ...
  it("Polaris projects canonical north through the same finite camera at extreme legal poses", () => {
    ...
    const north = polarisProjection({ generation: 1, pose: home, viewport, bodies: [], center: { x: 200, y: 300 } }, extent);
    expect(north.y).toBeLessThan(300);
    const yaw = polarisProjection({ generation: 1, pose: { ...home, yaw: Math.PI / 2 }, viewport, bodies: [], center: { x: 200, y: 300 } }, extent);
    expect(yaw.x).toBeGreaterThan(200);
    for (const e of [1, 20, 500, 10000]) {
      const pose = clampCameraPose({ x: e, y: e, zoom: 4, tilt: Math.PI / 3, yaw: Math.PI }, Math.max(e, 32));
      const point = polarisProjection({ generation: 1, pose, viewport, bodies: [], center: projectWorldPoint({ x: 0, y: 0 }, pose, viewport) }, e);
