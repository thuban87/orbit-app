...
describe("applyRestore", () => {
  ...
  it("recomputes a retained contact after inserting history and removes it from Not Contacted", async () => {
    ...
    const before = await readOrrerySystemSnapshot(destination, {
      kind: "builtin",
      id: "not-contacted",
    });
    ...
    await expect(
      applyRestore(destination, manifest, "merge"),
    ).resolves.toMatchObject({ status: "applied", inserted: 1, updated: 0 });
    await expect(
      destination.getFirstAsync(
        "SELECT name,interval_days,last_contact,modified_at FROM contacts WHERE uid='same-contact'",
      ),
    ).resolves.toEqual({
      name: "Newer local name",
      interval_days: 21,
    ...
    expect(
      (
        await readOrrerySystemSnapshot(destination, {
          kind: "builtin",
          id: "not-contacted",
        })
      ).members,
    ...
    await expect(
      applyRestore(destination, manifest, "merge"),
    ).resolves.toMatchObject({ inserted: 0, updated: 0, deleted: 0 });
    await expect(
      destination.getFirstAsync(
        "SELECT modified_at FROM contacts WHERE uid='same-contact'",
      ),
  ...
  it.each(["lower-newest", "disconnect", "delete", "reparent"] as const)(
    ...
    async (operation) => {
      ...
      if (operation === "lower-newest")
        await recordTouchpoint(destination, {
      ...
      manifest.contacts = manifest.contacts.map((row) => ({
        ...row,
        name: "Losing incoming name",
        modifiedAt: older,
      }));
      ...
      if (operation === "delete") {
        ...
        manifest.interactions = manifest.interactions.map((row) =>
          row.uid !== "latest"
            ? row
            : {
                ...row,
                modifiedAt: changedAt,
                ...(operation === "lower-newest"
      ...
      await expect(
        applyRestore(destination, parseBackupManifest(manifest), "merge"),
      ).resolves.toMatchObject({
        status: "applied",
        updated: operation === "delete" ? 0 : 1,
        deleted: operation === "delete" ? 1 : 0,
      });
      await expect(
        destination.getAllAsync(
          "SELECT uid,name,last_contact,modified_at FROM contacts ORDER BY uid",
        ),
      ).resolves.toEqual([
        {
          uid: "parent-a",
          name: "Local A",
          last_contact: operation === "lower-newest" ? older : null,
          modified_at: NOW,
        },
        {
          uid: "parent-b",
          name: "Local B",
          last_contact: operation === "reparent" ? newest : null,
          modified_at: NOW,
        },
      ]);
      const notContacted = await readOrrerySystemSnapshot(destination, {
        kind: "builtin",
        id: "not-contacted",
      });
      ...
      if (operation === "delete")
        await expect(
          destination.getFirstAsync(
            "SELECT deleted_at FROM tombstones WHERE entity_type='interaction' AND entity_uid='latest'",
          ),
  ...
  it("recomputes changed contact qualification against retained history with the incoming metadata timestamp", async () => {
    ...
    await expect(
      applyRestore(destination, manifest, "merge"),
    ).resolves.toMatchObject({ status: "applied", updated: 1 });
    await expect(
      destination.getFirstAsync(
        "SELECT name,rarely_responds,last_contact,modified_at FROM contacts WHERE uid='qualification'",
      ),
    ).resolves.toEqual({
      name: "After",
      rarely_responds: 1,
  ...
  it("recomputes the surviving destination when the reparented interaction's old parent is deleted", async () => {
    ...
    manifest.contacts = manifest.contacts.filter(
      (row) => row.uid === "surviving-parent",
    );
    manifest.interactions[0] = {
      ...manifest.interactions[0],
      contactUid: "surviving-parent",
      modifiedAt: "2026-08-26 12:00:00",
    };
    manifest.tombstones.push({
      entityType: "contact",
      entityUid: "removed-parent",
      deletedAt: "2026-08-26 12:00:00",
    });
    await expect(
      applyRestore(destination, parseBackupManifest(manifest), "merge"),
    ).resolves.toMatchObject({ status: "applied", deleted: 1, updated: 1 });
    await expect(
      destination.getAllAsync(
        "SELECT uid,last_contact,modified_at FROM contacts",
      ),
    ).resolves.toEqual([
  ...
  it("rolls back changed history, recency, and tombstones if a later restore write fails", async () => {
    ...
    await expect(applyRestore(destination, manifest, "merge")).rejects.toThrow(
      "restore revision failed",
    );
    await expect(
      destination.getFirstAsync(
        "SELECT name,last_contact,modified_at FROM contacts WHERE uid='atomic'",
      ),
    ).resolves.toEqual({ name: "Local", last_contact: NOW, modified_at: NOW });
    await expect(
      destination.getAllAsync("SELECT uid FROM interactions"),
    ).resolves.toEqual([{ uid: "deleted-touch" }]);
    await expect(
  ...
  it("round-trips live and deleted knowledge, linked people, and current/history state", async () => {
    ...
    const owner = await source.runAsync(
      "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
      ["knowledge-owner", "Owner", 14, 0, 0, NOW, NOW],
    );
    const linked = await source.runAsync(
      "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
      ["knowledge-linked", "Linked", 14, 0, 0, NOW, NOW],
    );
    await source.runAsync(
      "INSERT INTO memories (uid,contact_id,type,custom_label,value,pinned,outdated,provenance,created_at,modified_at,deleted_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
      [
        "memory-live",
    ...
    await source.runAsync(
      "INSERT INTO relationships (uid,contact_id,person_name,linked_contact_id,pinned,created_at,modified_at,deleted_at) VALUES (?,?,?,?,?,?,?,?)",
      [
        "relationship-linked",
        owner.lastInsertRowId,
        "Linked",
    ...
    await expect(
      destination.getAllAsync(
        "SELECT uid,deleted_at FROM memories ORDER BY uid",
      ),
    ).resolves.toEqual([
      { uid: "memory-deleted", deleted_at: "2026-08-26 12:00:00" },
      { uid: "memory-live", deleted_at: null },
    ]);
    await expect(
      destination.getFirstAsync<{ linked_uid: string }>(
        "SELECT linked.uid AS linked_uid FROM relationships r JOIN contacts linked ON linked.id=r.linked_contact_id WHERE r.uid=?",
        ["relationship-linked"],
      ),
    ).resolves.toEqual({ linked_uid: "knowledge-linked" });
    await expect(
      destination.getAllAsync(
        "SELECT uid,is_current FROM current_state_entries ORDER BY uid",
      ),
    ).resolves.toEqual([
      { uid: "state-current", is_current: 1 },
      { uid: "state-history", is_current: 0 },
    ]);
  });
  it("reconciles all decisions before one transaction and UID-maps an incoming contact", async () => {
    ...
    await expect(
      destination.getFirstAsync<{ name: string; interval_days: number }>(
        "SELECT name, interval_days FROM contacts WHERE uid = ?",
        ["incoming-contact"],
      ),
  ...
  it("round-trips an Unbound lifecycle state with its dormant cadence", async () => {
    ...
    await expect(
      ...
      }>("SELECT tracking_enabled,interval_days FROM contacts WHERE uid=?", [
        "unbound-contact",
      ]),
  ...
  it("remaps legacy interaction vocabulary on restore ingest via the shared map (D-06 backdoor)", async () => {
    ...
    expect(
      manifest.interactions.find((i) => i.uid === "i-good-text"),
    ).toMatchObject({ quality: "good", channel: "text" });
  ...
  it("forces allow_ai=0 on the merge/update arm — restore never keeps an interaction AI-permissive (D-04)", async () => {
    ...
    await expect(
      destination.getFirstAsync<{ allow_ai: number }>(
        "SELECT allow_ai FROM interactions WHERE uid=?",
        ["consent-i"],
      ),
  ...
  it("retains assigned local cadence when a newer valid Unbound merge proposes null cadence", async () => {
    ...
    await expect(
      destination.getFirstAsync<{
        ...
        "SELECT name,tracking_enabled,interval_days FROM contacts WHERE uid=?",
        ["shared-contact"],
      ),
    ).resolves.toEqual({
      name: "Incoming",
  ...
  it("uses an FK-safe reset for replace-all before inserting the incoming graph", async () => {
    ...
    await expect(
      applyRestore(destination, manifest, "replace-all"),
    ).resolves.toMatchObject({ status: "applied", mode: "replace-all" });
    await expect(
      destination.getAllAsync<{ uid: string }>(
        "SELECT uid FROM contacts ORDER BY uid",
      ),
    ).resolves.toEqual([{ uid: "replacement" }]);
    await expect(
      destination.getFirstAsync<{ entity_uid: string }>(
    ...
    await expect(
      destination.getAllAsync<{ entity_type: string; entity_uid: string }>(
  ...
  it("stages only a winning incoming photo, journals it in the transaction, and reports a retryable finalize failure", async () => {
    ...
    expect(photoMocks.staged).toEqual([
      ["YQ==", "avatars/_restore_pending/contact-photo-contact-session.jpg"],
    ]);
    await expect(
      destination.getFirstAsync<{ action: string }>(
        "SELECT action FROM restore_photo_journal",
      ),
    ).resolves.toEqual({ action: "finalize" });
    await expect(
      destination.getFirstAsync<{ photo: string }>(
        "SELECT photo FROM contacts WHERE uid=?",
        ["photo-contact"],
      ),
    ).resolves.toEqual({ photo: "avatars/contact-1.jpg" });
  });
  ...
  it("blocks a configured Replace-all until its forced verified safety snapshot succeeds", async () => {
    ...
    await expect(
      applyRestore(destination, manifest, "replace-all"),
    ).resolves.toEqual({ status: "pre-restore-snapshot-failed" });
    await expect(
      destination.getFirstAsync<{ count: number }>(
  ...
  it("commits restore while exposing schedule resync failures for the launch sweep to heal", async () => {
    ...
    await expect(
      destination.getFirstAsync<{ uid: string }>(
        "SELECT uid FROM contacts WHERE uid=?",
        ["schedule-contact"],
      ),
    ).resolves.toEqual({ uid: "schedule-contact" });
  });
  ...
  it("round-trips normalized method metadata without scalar contact columns", async () => {
    ...
    await expect(
      destination.getFirstAsync<{
        ...
        "SELECT canonical_region,label,extension FROM contact_methods WHERE uid=?",
        ["method-a"],
      ),
    ).resolves.toEqual({
      canonical_region: "US",
  ...
  it("demotes a retained primary before inserting a lexically earlier replacement primary", async () => {
    ...
    for (const exec of [source, destination]) {
      await exec.runAsync(
        "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
        ["shared-contact", "Shared", 14, 0, 0, NOW, "2026-08-25 12:00:00"],
      );
    }
    const sourceContact = await source.getFirstAsync<{ id: number }>(
      "SELECT id FROM contacts WHERE uid=?",
    ...
    await source.runAsync(
      "INSERT INTO contact_methods (uid,contact_id,method_type,raw_value,display_value,is_actionable,is_primary,display_order,created_at,modified_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
      [
        "a-promoted",
        sourceContact!.id,
        "phone",
    ...
    await expect(
      destination.getAllAsync<{ uid: string; is_primary: number }>(
        "SELECT uid,is_primary FROM contact_methods ORDER BY uid",
      ),
    ).resolves.toEqual([
      { uid: "a-promoted", is_primary: 1 },
      { uid: "z-retained", is_primary: 0 },
    ]);
  });
  ...
  it("removes a normalized method only when an explicit newer tombstone says so", async () => {
    ...
    for (const exec of [source, destination]) {
      await exec.runAsync(
        "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
        ["tombstone-contact", "Tombstone", 14, 0, 0, NOW, NOW],
      );
    ...
    await expect(
      applyRestore(destination, manifest, "merge"),
    ).resolves.toMatchObject({ status: "applied", deleted: 1 });
    await expect(
      destination.getFirstAsync("SELECT uid FROM contact_methods WHERE uid=?", [
        "tombstone-method",
      ]),
    ).resolves.toBeNull();
  });
  ...
  it("removes Memory and relationship rows when a newer tombstone says so", async () => {
    ...
    manifest.tombstones = [
      {
        entityType: "memory",
        entityUid: "tombstone-memory",
        deletedAt: "2026-08-25 12:01:00",
      },
      {
        entityType: "relationship",
        entityUid: "tombstone-relationship",
        deletedAt: "2026-08-25 12:01:00",
      },
    ];
    ...
    await expect(
      applyRestore(destination, manifest, "merge"),
    ).resolves.toMatchObject({ status: "applied", deleted: 2 });
    await expect(
      destination.getAllAsync(
        "SELECT uid FROM memories WHERE uid='tombstone-memory'",
      ),
    ).resolves.toEqual([]);
    await expect(
      destination.getAllAsync(
        "SELECT uid FROM relationships WHERE uid='tombstone-relationship'",
      ),
    ).resolves.toEqual([]);
  });
  ...
  it("requires full contact×def pairs only for global-scope defs, exempting contact-scoped defs", async () => {
    ...
    for (const name of ["pair-c1", "pair-c2"]) {
      await globalSource.runAsync(
        "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
        [name, name, 14, 0, 0, NOW, NOW],
      );
    }
    const c1 = await globalSource.getFirstAsync<{ id: number }>(
      "SELECT id FROM contacts WHERE uid=?",
      ["pair-c1"],
    );
    await globalSource.runAsync(
      "INSERT INTO custom_field_defs (uid,col_name,label,type,show_on_new,always_show,display_order,share_with_ai,scope,created_at,modified_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
      [
        "global-def",
        "nickname",
        "Nickname",
    ...
    const gdef = await globalSource.getFirstAsync<{ id: number }>(
    ...
    await globalSource.runAsync(
      ...
      ["gval-1", c1!.id, gdef!.id, "Ada", NOW, NOW],
    );
    const globalManifest = await buildExportManifest(globalSource, {
      exportedAt: NOW,
      readPhotoBase64: async () => "",
    });
    // A global def with only one of two contacts' pairs is still rejected.
    await expect(
      applyRestore(await db(), globalManifest, "merge"),
    ).rejects.toThrow(/missing a normalized custom-field value pair/);

    const scopedSource = await db();
    for (const name of ["scoped-c1", "scoped-c2"]) {
      await scopedSource.runAsync(
        "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
        [name, name, 14, 0, 0, NOW, NOW],
      );
    }
    const s1 = await scopedSource.getFirstAsync<{ id: number }>(
      "SELECT id FROM contacts WHERE uid=?",
      ["scoped-c1"],
    );
    await scopedSource.runAsync(
    ...
    const sdef = await scopedSource.getFirstAsync<{ id: number }>(
      "SELECT id FROM custom_field_defs WHERE uid=?",
      ["scoped-def"],
    );
    await scopedSource.runAsync(
      "INSERT INTO custom_field_values (uid,contact_id,field_def_id,value,created_at,modified_at) VALUES (?,?,?,?,?,?)",
      ["sval-1", s1!.id, sdef!.id, "Choice", NOW, NOW],
    );
    const scopedManifest = await buildExportManifest(scopedSource, {
      exportedAt: NOW,
      readPhotoBase64: async () => "",
    });
    ...
    await expect(
      applyRestore(scopedDest, scopedManifest, "merge"),
    ).resolves.toMatchObject({ status: "applied" });
    await expect(
      scopedDest.getFirstAsync<{ uid: string; scope: string }>(
        "SELECT uid,scope FROM custom_field_defs WHERE uid=?",
        ["scoped-def"],
      ),
    ).resolves.toEqual({ uid: "scoped-def", scope: "contact" });
  });

  it("round-trips the portable phone region override through export and restore", async () => {
    const source = await db();
    await source.runAsync(
      "UPDATE app_settings SET phone_region_override=?, modified_at=? WHERE id=1",
      ["GB", "2026-08-25 12:01:00"],
    );
    const manifest = await buildExportManifest(source, {
      exportedAt: NOW,
      readPhotoBase64: async () => "",
    ...
    await expect(
      destination.getFirstAsync<{ phone_region_override: string }>(
  ...
  it("restores the declare-only camelCase channel keys into their SQLite columns via COLUMN_OF (CAPT-11)", async () => {
    ...
    (
      manifest.appSettings as Record<string, unknown>
    ...
    await expect(
      ...
      default_interaction_channel: "Call",
      remembered_interaction_channel: "In Person",
    });
  });
  ...
  it("rejects an out-of-vocabulary restored channel value before writing (CAPT-11, T-34-03)", async () => {
    ...
    (
      manifest.appSettings as Record<string, unknown>
    ).defaultInteractionChannel = "totally-bogus";
    const destination = await db();
    ...
    await expect(
      destination.getFirstAsync<{ default_interaction_channel: string }>(
  ...
  it("round-trips allow_ai, scoped/history/group defs, value history, soft-deletes, and a history tombstone in merge", async () => {
    ...
    const owner = await source.runAsync(
      "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
      ["know-owner", "Owner", 14, 0, 0, NOW, NOW],
    );
    const ownerId = owner.lastInsertRowId;
    // (a) memories: allow_ai ON; imported-type (allow_ai OFF, KNOW-14 provenance); soft-deleted.
    await source.runAsync(
      "INSERT INTO memories (uid,contact_id,type,value,pinned,outdated,provenance,allow_ai,created_at,modified_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
      [
        "mem-allow",
        ownerId,
        "general",
    ...
    await source.runAsync(
      "INSERT INTO memories (uid,contact_id,type,custom_label,value,pinned,outdated,provenance,allow_ai,created_at,modified_at,deleted_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
      [
        "mem-deleted",
        ownerId,
        "custom",
    ...
    await source.runAsync(
      "INSERT INTO custom_field_defs (uid,col_name,label,type,show_on_new,always_show,display_order,share_with_ai,scope,history_retained,field_group,created_at,modified_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
      [
        "g-def",
        "nickname",
        "Nickname",
    ...
    const gDef = await source.getFirstAsync<{ id: number }>(
      "SELECT id FROM custom_field_defs WHERE uid=?",
      ["g-def"],
    );
    const sDef = await source.getFirstAsync<{ id: number }>(
      "SELECT id FROM custom_field_defs WHERE uid=?",
      ["s-def"],
    );
    await source.runAsync(
      "INSERT INTO custom_field_values (uid,contact_id,field_def_id,value,created_at,modified_at) VALUES (?,?,?,?,?,?)",
      ["g-val", ownerId, gDef!.id, "Ace", NOW, NOW],
    );
    await source.runAsync(
      "INSERT INTO custom_field_values (uid,contact_id,field_def_id,value,created_at,modified_at) VALUES (?,?,?,?,?,?)",
      ["s-val", ownerId, sDef!.id, "Current", NOW, NOW],
    );
    ...
    await source.runAsync(
      ...
      ["hist-1", ownerId, sDef!.id, "Older", NOW],
    );
    await source.runAsync(
      "INSERT INTO custom_field_value_history (uid,contact_id,field_def_id,value,created_at) VALUES (?,?,?,?,?)",
      ["hist-2", ownerId, sDef!.id, "Oldest", "2026-08-24 12:00:00"],
    );
    // (d) a value-history tombstone that must survive export→restore.
    await source.runAsync(
      "INSERT INTO tombstones (entity_type,entity_uid,deleted_at) VALUES (?,?,?)",
      ["custom_field_value_history", "hist-ghost", NOW],
    );
    ...
    await expect(
      ...
      }>("SELECT uid,allow_ai,deleted_at FROM memories ORDER BY uid"),
    ).resolves.toEqual([
    ...
    await expect(
      destination.getFirstAsync<{ type: string }>(
        "SELECT type FROM memories WHERE uid=?",
        ["mem-imported"],
      ),
    ).resolves.toEqual({ type: "imported" });
    await expect(
      destination.getAllAsync<{
        uid: string;
        scope: string;
        ...
        "SELECT uid,scope,history_retained,field_group FROM custom_field_defs ORDER BY uid",
      ),
    ).resolves.toEqual([
    ...
    await expect(
      destination.getAllAsync<{ uid: string; value: string }>(
        "SELECT uid,value FROM custom_field_value_history ORDER BY uid",
      ),
    ).resolves.toEqual([
    ...
    await expect(
      destination.getFirstAsync<{ entity_uid: string }>(
        "SELECT entity_uid FROM tombstones WHERE entity_type='custom_field_value_history' AND entity_uid='hist-ghost'",
      ),
    ).resolves.toEqual({ entity_uid: "hist-ghost" });
  });
  ...
  it("clears destination-only value history on replace-all before writing the manifest's rows", async () => {
    ...
    const raDef = await source.getFirstAsync<{ id: number }>(
      "SELECT id FROM custom_field_defs WHERE uid=?",
      ["ra-def"],
    );
    await source.runAsync(
      "INSERT INTO custom_field_values (uid,contact_id,field_def_id,value,created_at,modified_at) VALUES (?,?,?,?,?,?)",
      ["ra-val", owner.lastInsertRowId, raDef!.id, "Now", NOW, NOW],
    );
    await source.runAsync(
      "INSERT INTO custom_field_value_history (uid,contact_id,field_def_id,value,created_at) VALUES (?,?,?,?,?)",
      ["ra-hist", owner.lastInsertRowId, raDef!.id, "Then", NOW],
    );
    const manifest = await buildExportManifest(source, {
      exportedAt: NOW,
      readPhotoBase64: async () => "",
    ...
    const destination = await db();
    const destContact = await destination.runAsync(
      "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
      ["dest-owner", "Dest", 14, 0, 0, NOW, NOW],
    );
    await destination.runAsync(
      "INSERT INTO custom_field_defs (uid,col_name,label,type,show_on_new,always_show,display_order,share_with_ai,scope,history_retained,field_group,created_at,modified_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
      ["dest-def", "x", "X", "text", 0, 0, 0, 0, "contact", 1, null, NOW, NOW],
    );
    const destDef = await destination.getFirstAsync<{ id: number }>(
      "SELECT id FROM custom_field_defs WHERE uid=?",
      ["dest-def"],
    );
    await destination.runAsync(
      "INSERT INTO custom_field_value_history (uid,contact_id,field_def_id,value,created_at) VALUES (?,?,?,?,?)",
      ["dest-hist", destContact.lastInsertRowId, destDef!.id, "Stale", NOW],
    );
    ...
    await expect(
      destination.getAllAsync<{ uid: string }>(
        "SELECT uid FROM custom_field_value_history ORDER BY uid",
      ),
    ).resolves.toEqual([{ uid: "ra-hist" }]);
  });
  ...
  it("parses a pre-phase format-4 backup end-to-end and restores defaulted knowledge columns (P07-MED)", async () => {
    ...
    const owner = await source.runAsync(
      "INSERT INTO contacts (uid,name,interval_days,rarely_responds,reminders_off,created_at,modified_at) VALUES (?,?,?,?,?,?,?)",
      ["old-owner", "Owner", 14, 0, 0, NOW, NOW],
    );
    await source.runAsync(
      "INSERT INTO memories (uid,contact_id,type,value,pinned,outdated,provenance,allow_ai,created_at,modified_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
      [
    ...
    const oldDef = await source.getFirstAsync<{ id: number }>(
      "SELECT id FROM custom_field_defs WHERE uid=?",
      ["old-def"],
    );
    await source.runAsync(
      "INSERT INTO custom_field_values (uid,contact_id,field_def_id,value,created_at,modified_at) VALUES (?,?,?,?,?,?)",
      ["old-val", owner.lastInsertRowId, oldDef!.id, "Ace", NOW, NOW],
    );
    const manifest = await buildExportManifest(source, {
      exportedAt: NOW,
      readPhotoBase64: async () => "",
    ...
    for (const def of older.customFieldDefs) {
      delete def.scope;
      delete def.historyRetained;
      delete def.fieldGroup;
    }
    ...
    const destination = await db();
    await expect(
      applyRestore(destination, parsed, "replace-all"),
    ).resolves.toMatchObject({ status: "applied" });
    await expect(
      destination.getFirstAsync<{ allow_ai: number }>(
        "SELECT allow_ai FROM memories WHERE uid=?",
        ["old-mem"],
      ),
    ).resolves.toEqual({ allow_ai: 0 });
    await expect(
      destination.getFirstAsync<{
        ...
        "SELECT scope,history_retained,field_group FROM custom_field_defs WHERE uid=?",
        ["old-def"],
      ),
    ).resolves.toEqual({
      scope: "global",
    ...
    await expect(
      destination.getFirstAsync<{ n: number }>(
        "SELECT COUNT(*) AS n FROM custom_field_value_history",
      ),
