...
const FORBIDDEN_KEYS = new Set([
  ...
  "backup_folder_uri",
  "backup_folder_name",
  "backup_folder_accessible",
  "backup_folder_diagnostic",
...
function assertNoLocalOnlyKeys(value: unknown): void {
  if (Array.isArray(value)) {
    for (const item of value) assertNoLocalOnlyKeys(item);
    return;
  }
  if (!value || typeof value !== "object") return;
  for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
    if (FORBIDDEN_KEYS.has(key))
      throw new Error("backup manifest contains a local-only field");
...
async function readManifest(
  ...
  const [
    ...
    ro.getAllAsync<Record<string, unknown>>(
      "SELECT l.uid, c.uid AS contactUid, l.provider, l.external_contact_id AS externalContactId, l.is_active AS isActive, l.created_at AS createdAt, l.modified_at AS modifiedAt FROM external_contact_links l JOIN contacts c ON c.id = l.contact_id ORDER BY l.uid",
    ),
    ro.getAllAsync<Record<string, unknown>>(
    ...
    ro.getAllAsync<Record<string, unknown>>(
      "SELECT f.uid, c.uid AS contactUid, f.kind, f.label, f.text, f.url, f.created_at AS createdAt, f.source, f.modified_at AS modifiedAt FROM fuel f JOIN contacts c ON c.id = f.contact_id ORDER BY f.uid",
    ),
    ro.getAllAsync<Record<string, unknown>>(
      "SELECT l.uid, c.uid AS contactUid, l.url, l.label, l.display_order AS displayOrder, l.created_at AS createdAt, l.modified_at AS modifiedAt FROM contact_links l JOIN contacts c ON c.id = l.contact_id ORDER BY l.uid",
    ),
    ro.getAllAsync<Record<string, unknown>>(
    ...
    ro.getAllAsync<Record<string, unknown>>(
      "SELECT s.uid,c.uid AS contactUid,s.field_key AS fieldKey,s.value,s.is_current AS isCurrent,s.created_at AS createdAt,s.modified_at AS modifiedAt FROM current_state_entries s JOIN contacts c ON c.id=s.contact_id ORDER BY s.uid",
    ),
    ro.getAllAsync<Record<string, unknown>>(
  ...
  const contactUid = new Map(
    contactRows.map((row) => [row.localContactId as number, row.uid as string]),
  );
  const sunContactUid =
    settings.sunContactId === null
      ? null
      : contactUid.get(settings.sunContactId);
  if (settings.sunContactId !== null && !sunContactUid)
    throw new Error("portable settings refer to an unknown sun contact");
  ...
  const manifest: BackupManifest = {
    ...
    metadata: {
      ...
      sqliteUserVersion: userVersion?.user_version ?? null,
    },
