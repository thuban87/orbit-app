...
export function MergeImpactSummary({ navigation, route }: RootStackScreenProps<"MergeImpactSummary">) {
  ...
  const confirm = () => Alert.alert("Merge contacts?", "This cannot be undone.", [{ text: "Cancel", style: "cancel" }, { text: "Merge", style: "destructive", onPress: () => { void mergeContacts(getExecutor(), { survivorId, absorbedId, resolutions: typedResolutions, now: localDateTime() }).then(() => navigationRef.current?.reset(resetToDashboardWith({ name: "Profile", params: { contactId: survivorId } }))).catch(() => setFailed(true)); } }]);
