...
export function MergeImpactSummary({ navigation, route }: RootStackScreenProps<"MergeImpactSummary">) {
  const { colors } = useTheme(); const { survivorId, absorbedId } = route.params;
  ...
  const confirm = () => Alert.alert("Merge contacts?", "This cannot be undone.", [{ text: "Cancel", style: "cancel" }, { text: "Merge", style: "destructive", onPress: () => { void mergeContacts(getExecutor(), { survivorId, absorbedId, now: localDateTime() }).then(() => navigation.replace("Profile", { contactId: survivorId })).catch(() => setFailed(true)); } }]);
