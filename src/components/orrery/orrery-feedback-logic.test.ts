...
import { feedbackCopy, createFeedbackRetry } from "./orrery-feedback-logic";
...
describe("surface-specific Orrery feedback", () => {
  it("keeps loading, missing, stale, and optional failures distinct", () => {
    ...
    expect(feedbackCopy("read")).toEqual({ message: "Couldn't load this System. Try loading it again.", action: "Reload System" });
    expect(feedbackCopy("stale").message).toContain("last loaded contacts");
    expect(feedbackCopy("satellites").message).toContain("Your contacts are still available.");
    expect(feedbackCopy("removed").action).toBe("Show All Contacts");
    expect(feedbackCopy("reorder").message).toContain("saved order has been restored");
  });
  it("suppresses a double tap and permits retry after rejection", async () => {
    let reject!: (error: Error) => void;
    const operation = vi.fn(() => new Promise<void>((_, no) => { reject = no; }));
