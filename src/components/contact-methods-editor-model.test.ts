...
function groups(): MethodGroups {
  return {
    phone: [
      { uid: "p1", type: "phone", value: "555 1000", extension: "12", label: "Mobile", isPrimary: true },
      { uid: "p2", type: "phone", value: "555 2000", extension: "", label: "Work" },
    ],
    email: [{ uid: "e1", type: "email", value: "a@example.com", extension: "", label: "Home" }],
  };
}

describe("contact method editor model", () => {
  it("discards blank rows but preserves nonblank invalid drafts for the aggregate write", () => {
    const drafts = [emptyMethodDraft("phone", "blank"), { ...emptyMethodDraft("phone", "invalid"), value: "not a number" }];
    expect(discardBlankMethodDrafts(drafts).map((row) => row.uid)).toEqual(["invalid"]);
    expect(toMethodDrafts({ phone: drafts, email: [] })).toEqual([
      { uid: "invalid", type: "phone", value: "not a number", extension: undefined, isPrimary: undefined },
    ]);
  });
  ...
  it("keeps stable order, extension and labels while adding or promoting primaries", () => {
    ...
    expect(toMethodDrafts(initial)[0]).toMatchObject({ uid: "p1", extension: "12", isPrimary: true });
    expect(choosePrimary(initial, "p2").phone.map((row) => row.isPrimary)).toEqual([false, true]);
    expect(removeMethodDraft(initial, "p1").phone).toMatchObject([{ uid: "p2", isPrimary: true }]);
  });
