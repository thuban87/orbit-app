...
async function seedContact(opts: {
  ...
  const res = await exec.runAsync(
    `INSERT INTO contacts
       (uid, name, interval_days, tracking_enabled, last_contact, archived_at, created_at, modified_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      uid(),
      opts.name,
...
describe("STATUS_SCAN", () => {
  ...
  it("excludes an Unbound contacted contact before nullable-cadence status SQL can run", async () => {
    await seedContact({
      name: "Dormant",
      intervalDays: 10,
    ...
    expect(await exec.getAllAsync<ScanRow>(STATUS_SCAN)).toEqual([]);
  });
});
