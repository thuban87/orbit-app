...
/** A user-surfaceable, non-destructive refusal under Group Event membership uniqueness. */
export class GroupMergeCollisionError extends Error {
  readonly code = "GROUP_MERGE_COLLISION";

  constructor() {
    super(
      "These contacts share a Group Event. Remove one from that Group Event before merging them.",
    );
    this.name = "GroupMergeCollisionError";
  }
}
...
export function normalizeMergeResolutions(input: unknown): MergeResolutions {
  ...
  if (
    raw.scalars &&
    typeof raw.scalars === "object" &&
    !Array.isArray(raw.scalars)
  ) {
    for (const key of [
      "name",
      "categoryId",
  ...
  if (
    ...
    for (const [key, value] of Object.entries(
      ...
      if (Number.isInteger(id) && id > 0 && normalized)
        customFields[id] = normalized;
    }
  }
  const primaryMethod: Partial<Record<ContactMethodType, MergeChoice>> = {};
  if (
    raw.primaryMethod &&
    typeof raw.primaryMethod === "object" &&
    !Array.isArray(raw.primaryMethod)
  ) {
    for (const type of ["phone", "email"] as const) {
      const value = choice(
        (raw.primaryMethod as Record<string, unknown>)[type],
      );
      if (value) primaryMethod[type] = value;
    }
  }
  const photo =
    raw.photo === "keep-survivor"
      ? "keep-survivor"
      : raw.photo &&
...
async function reparent(
  ...
  await exec.runAsync(
    `UPDATE ${table} SET contact_id = ?, modified_at = ? WHERE contact_id = ?`,
...
export async function mergeContacts(
  ...
  await inWriteTransaction(exec, async () => {
    ...
    const survivor = await exec.getFirstAsync<ContactRow>(
      `SELECT ${columns} FROM contacts WHERE id = ?`,
      [input.survivorId],
    );
    const absorbed = await exec.getFirstAsync<ContactRow>(
      `SELECT ${columns} FROM contacts WHERE id = ?`,
      [input.absorbedId],
    );
    if (
      !survivor ||
      survivor.archived_at !== null ||
      !absorbed ||
      absorbed.archived_at !== null
    ) {
    ...
    for (const type of ["phone", "email"] as const) {
      const own = methods.filter(
        (row) =>
          ...
          row.method_type === type &&
          row.is_primary === 1,
      );
      const other = methods.filter(
        (row) =>
          row.contact_id === absorbed.id &&
          row.method_type === type &&
          row.is_primary === 1,
      );
      ...
      if (resolutions.primaryMethod?.[type] === "absorbed") {
        await exec.runAsync(
          "UPDATE contact_methods SET is_primary = 0, modified_at = ? WHERE id = ?",
          [input.now, own[0].id],
        );
      } else {
        await exec.runAsync(
          "UPDATE contact_methods SET is_primary = 0, modified_at = ? WHERE id = ?",
          [input.now, other[0].id],
        );
      }
    }
    const links = await exec.getAllAsync<{
      id: number;
      provider: string;
      external_contact_id: string;
    }>(
      "SELECT id, provider, external_contact_id FROM external_contact_links WHERE contact_id = ? AND is_active = 1",
      [survivor.id],
    );
    const absorbedLinks = await exec.getAllAsync<{
      id: number;
      provider: string;
      external_contact_id: string;
    }>(
    ...
    for (const link of absorbedLinks)
      if (
        links.some(
          (own) =>
            own.provider === link.provider &&
            own.external_contact_id === link.external_contact_id,
        )
      ) {
        await exec.runAsync(
          "UPDATE external_contact_links SET is_active = 0, modified_at = ? WHERE id = ?",
          [input.now, link.id],
        );
      }
    for (const absorbedMethod of methods.filter(
      (row) => row.contact_id === absorbed.id && row.canonical_value,
    )) {
      const duplicate = methods.find(
        (row) =>
          row.contact_id === survivor.id &&
          row.method_type === absorbedMethod.method_type &&
          row.canonical_value === absorbedMethod.canonical_value,
      );
      if (!duplicate) continue;
      const selectedAbsorbed =
        resolutions.primaryMethod?.[absorbedMethod.method_type] ===
          "absorbed" && absorbedMethod.is_primary === 1;
      const kept = selectedAbsorbed ? absorbedMethod : duplicate;
      const losing = selectedAbsorbed ? duplicate : absorbedMethod;
      await exec.runAsync(
        "UPDATE contact_method_provenance SET method_id = ?, modified_at = ? WHERE method_id = ?",
        [kept.id, input.now, losing.id],
      );
      await exec.runAsync("DELETE FROM contact_methods WHERE id = ?", [
        losing.id,
      ]);
    }
    ...
    const survivorValues = await exec.getAllAsync<{
      ...
      "SELECT field_def_id, value FROM custom_field_values WHERE contact_id = ?",
      [survivor.id],
    );
    const absorbedValues = await exec.getAllAsync<{
      field_def_id: number;
      value: string | null;
    }>(
      "SELECT field_def_id, value FROM custom_field_values WHERE contact_id = ?",
    ...
    for (const other of absorbedValues) {
      ...
      const takeAbsorbed =
        resolutions.customFields?.[other.field_def_id] === "absorbed" ||
      ...
      if (takeAbsorbed) {
        await snapshot(
          exec,
          survivor.id,
          `custom_field:${other.field_def_id}`,
          own.value,
          "merge-overwritten",
          input.now,
        );
    ...
    // Like the current_state_entries partial-unique pre-handling above, detect
    // an interaction constraint collision before the blanket reparent. ADR-010
    // recency remains correct on the no-collision path via the existing single
    // recomputeLastContactCore(survivor) at the end of this transaction.
    const sharedGroup = await exec.getFirstAsync<{ found: number }>(
      `SELECT 1 AS found
         FROM interactions AS a
         JOIN interactions AS b ON b.group_event_id = a.group_event_id
        WHERE a.contact_id = ?
          AND b.contact_id = ?
          AND a.group_event_id IS NOT NULL
        LIMIT 1`,
      [survivor.id, absorbed.id],
    );
    if (sharedGroup) throw new GroupMergeCollisionError();
    ...
    for (const table of [
      ...
      await reparent(exec, table, survivor.id, absorbed.id, input.now);
    await exec.runAsync(
      "UPDATE field_history SET contact_id = ? WHERE contact_id = ?",
      [survivor.id, absorbed.id],
    );
    await exec.runAsync(
      "UPDATE custom_field_value_history SET contact_id = ? WHERE contact_id = ?",
      [survivor.id, absorbed.id],
    );

    const scalar = resolutions.scalars ?? {};
    const fields: Array<
      [keyof typeof scalar, keyof ContactRow, keyof UpdateContactFullInput]
    > = [
      ["name", "name", "name"],
      ["categoryId", "category_id", "categoryId"],
      ["intervalDays", "interval_days", "intervalDays"],
      ["trackingEnabled", "tracking_enabled", "trackingEnabled"],
      ["socialBattery", "social_battery", "socialBattery"],
      ["birthday", "birthday", "birthday"],
      ["rarelyResponds", "rarely_responds", "rarelyResponds"],
      ["remindersOff", "reminders_off", "remindersOff"],
    ];
    const next: UpdateContactFullInput = {
      id: survivor.id,
      name: survivor.name,
      categoryId: survivor.category_id,
      intervalDays: survivor.interval_days,
      trackingEnabled: survivor.tracking_enabled === 1,
      socialBattery: survivor.social_battery,
      birthday: survivor.birthday,
      rarelyResponds: survivor.rarely_responds,
      remindersOff: survivor.reminders_off,
      now: input.now,
    };
    for (const [resolutionKey, rowKey, inputKey] of fields) {
      ...
      await snapshot(
        ...
        survivor[rowKey] as string | number | null,
        "merge-overwritten",
        input.now,
      );
    ...
    if (
      Object.keys(scalar).length ||
      fields.some(
        ([resolutionKey, rowKey]) =>
          scalar[resolutionKey] == null &&
          isEmptyMergeValue(survivor[rowKey] as string | number | null) &&
          !isEmptyMergeValue(absorbed[rowKey] as string | number | null),
      )
    ...
    for (const [field, value] of [
      ["favourite_rank", absorbed.favourite_rank],
      ["ring_seq", absorbed.ring_seq],
      ["snooze_until", absorbed.snooze_until],
    ] as const)
      if (value != null)
        await snapshot(
          exec,
    ...
    const cards = await exec.getAllAsync<{ session_id: number }>(
      "SELECT DISTINCT session_id FROM reconciliation_session_cards WHERE contact_id = ?",
      [absorbed.id],
    );
    await exec.runAsync(
      "DELETE FROM reconciliation_session_cards WHERE contact_id = ?",
      [absorbed.id],
    );
    for (const card of cards) {
      const remaining = await exec.getFirstAsync<{ n: number }>(
        "SELECT COUNT(*) AS n FROM reconciliation_session_cards WHERE session_id = ? AND card_status NOT IN ('resolved', 'missing_source')",
        [card.session_id],
      );
      if (remaining?.n === 0)
        await exec.runAsync(
          "UPDATE reconciliation_sessions SET status = 'complete', modified_at = ? WHERE id = ?",
          [input.now, card.session_id],
        );
    }
    await insertTombstoneCore(exec, {
      entityType: "contact",
      entityUid: absorbed.uid,
      deletedAt: input.now,
    });
