...
async function edit(contactId: number, fieldDefId: number, value: string | null, now = LATER): Promise<void> {
...
describe("retained custom-field value history", () => {
  ...
  it("records changing prior raw values through the real updateContactFull edit path", async () => {
    ...
    await createField(exec, {
      uid: uid(), col_name: "note", label: "Note", type: "text", options: null,
      show_on_new: 0, always_show: 0, display_order: 0, share_with_ai: 0,
      history_retained: 1, now: NOW,
    });
    const def = await exec.getFirstAsync<{ id: number }>("SELECT id FROM custom_field_defs WHERE col_name = 'note'");
    ...
    expect((await listValueHistory(exec, contactId, def.id)).map((row) => row.value))
      .toEqual(["next", "raw  x"]);
  });
  ...
  it("never appends a non-history definition", async () => {
    ...
    await createField(exec, {
      uid: uid(), col_name: "plain", label: "Plain", type: "text", options: null,
      show_on_new: 0, always_show: 0, display_order: 0, share_with_ai: 0, now: NOW,
    });
    const def = await exec.getFirstAsync<{ id: number }>("SELECT id FROM custom_field_defs WHERE col_name = 'plain'");
    if (!def) throw new Error("missing field definition");
  ...
  it("rejects a second contact writing a directly-present contact-scoped def", async () => {
    const owner = await createContactFull(exec, { uid: uid(), name: "Owner", intervalDays: 30, now: NOW });
    const other = await createContactFull(exec, { uid: uid(), name: "Other", intervalDays: 30, now: NOW });
    const def = await exec.runAsync(
      ...
      [uid(), "private", "Private", "text", null, 0, 0, 0, 0, "contact", 0, null, NOW, NOW],
    );
    await edit(owner.contactId, def.lastInsertRowId, "owner value");
    await expect(edit(other.contactId, def.lastInsertRowId, "not allowed"))
  ...
  it("rolls back both the current-value update and appended history together", async () => {
    ...
    await createField(exec, {
      uid: uid(), col_name: "atomic", label: "Atomic", type: "text", options: null,
      show_on_new: 0, always_show: 0, display_order: 0, share_with_ai: 0,
      history_retained: 1, now: NOW,
    });
    const def = await exec.getFirstAsync<{ id: number }>("SELECT id FROM custom_field_defs WHERE col_name = 'atomic'");
    if (!def) throw new Error("missing field definition");
    await edit(contactId, def.id, "before");
    await expect(updateContactFull(exec, {
      id: contactId, name: "Alex", intervalDays: 30, rarelyResponds: 0, remindersOff: 0,
      now: "2026-09-04 12:02:00",
      customValues: [{ fieldDefId: def.id, value: "after" }, { fieldDefId: 999999, value: "fail" }],
    })).rejects.toThrow();
    expect(await exec.getFirstAsync(
      "SELECT value FROM custom_field_values WHERE contact_id = ? AND field_def_id = ?",
      [contactId, def.id],
