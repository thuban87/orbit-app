...
export async function listContactMethodGroups(exec: SqlExecutor, contactId: number): Promise<ContactMethodGroups> {
  ...
  return { phone: rows.filter((row) => row.method_type === "phone"), email: rows.filter((row) => row.method_type === "email") };
}
 ...
 */
export async function listActionablePrimaryMethods(exec: SqlExecutor, contactId: number): Promise<{ phone: ContactMethodRow | null; email: ContactMethodRow | null }> {
  const groups = await listContactMethodGroups(exec, contactId);
  const select = (rows: ContactMethodRow[]) => rows.find((row) => row.is_primary === 1 && row.is_actionable === 1) ?? rows.find((row) => row.is_actionable === 1) ?? null;
