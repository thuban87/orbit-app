...
- **Prerequisite anchor-phase check (batch-order guard).** Before planning a CREATE for a system doc, look up its *anchor phase(s)* in the `docs/systems/README.md` routing table. If an **earlier** anchor phase than the one you are extracting owns that doc and the doc does **not** exist yet, the batch is running out of order — that earlier phase should have CREATED it. Do **not** silently CREATE: stop and warn loudly (`PREREQUISITE MISSING: docs/systems/<name>.md is anchored to phase NN (earlier than this phase) but does not exist — extract phase NN first`), and only CREATE if the owner confirms the out-of-order run is intended. (In a correct strict-order batch this never fires.)
